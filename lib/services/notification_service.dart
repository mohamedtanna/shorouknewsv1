...
class NotificationService {
  ...
  Future<void> showNotification({
    ...
    String channelId =
        isBreakingNews ? _breakingNewsChannelId : _defaultChannelId;
    ...
    androidDetails = AndroidNotificationDetails(
      ...
      styleInformation: bigPictureBitmap != null
          ? BigPictureStyleInformation(
              bigPictureBitmap,
              largeIcon:
                  bigPictureBitmap, // Can also use a different icon for largeIcon
              contentTitle: title,
          ...
          : BigTextStyleInformation(
              // Default to BigText if no image
              body,
      ...
      actions: <AndroidNotificationAction>[
        const AndroidNotificationAction('read_action', 'قراءة',
            showsUserInterface: true),
        const AndroidNotificationAction('dismiss_action', 'تجاهل',
  ...
  Future<void> scheduleNotification({
    ...
    String?
        imageUrl, // imageUrl for scheduled notifications might be complex due to caching/validity
    Map<String, dynamic>? data,
    NotificationPriorityLevel priority = NotificationPriorityLevel.normal,
  }) async {
    if (!_isInitialized) await initialize();
    ...
    try {
      await _flutterLocalNotificationsPlugin.zonedSchedule(
        ...
        matchDateTimeComponents:
            DateTimeComponents.time, // Or .dateAndTime depending on needs
      );
      debugPrint(
          'Notification scheduled for: $tzScheduledDate, ID: $notificationId');
    } catch (e) {
  ...
  static void _handleNotificationNavigation(
      NotificationPayload payload, String? actionId) {
    ...
    try {
      ...
      debugPrint(
          "Error navigating from notification: $e. Path: $navigationPath");
  ...
  Importance _getImportance(NotificationPriorityLevel priority) {
    switch (priority) {
      ...
      case NotificationPriorityLevel.normal:
  ...
  Future<void> _loadNotificationHistory() async {
    try {
      ...
      final historyJson =
          prefs.getString('notification_history_v1'); // Versioned key
      if (historyJson != null) {
        ...
        _notificationHistory = historyList
            ...
            .toList();
        debugPrint(
            '${_notificationHistory.length} notifications loaded from history.');
      }
    } catch (e) {
  ...
  Future<void> _saveNotificationHistory() async {
    try {
      ...
      final historyJson =
          jsonEncode(_notificationHistory.map((n) => n.toJson()).toList());
  ...
  Future<void> openPlatformNotificationSettings() async {
    ...
    debugPrint(
        "Attempted to open platform notification settings (manual implementation may be needed via other plugins for direct navigation).");
  }
  ...
  Future<void> sendTestNotification() async {
    await showNotification(
        ...
        imageUrl:
            'https://via.placeholder.com/400x200.png?text=Test+Image' // Test image
        );
  }

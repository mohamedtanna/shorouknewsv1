...
class ColumnsModule {
  ...
  static const String _selectedColumnsEndpoint =
  ...
  Stream<ColumnEvent> get eventStream =>
  ...
  Future<List<ColumnModel>> getSelectedColumns(
  ...
  Future<ColumnModel> getColumnDetails(String cdate, String columnId,
      {bool forceRefresh = false}) async {
    _ensureInitialized();
  ...
  Future<void> _saveFavorites() async {
    await _prefs?.setStringList(
        _favoriteColumnsKey, _favoriteColumnIds.toList());
  }
  ...
  Future<void> _saveBookmarks() async {
    await _prefs?.setStringList(
        _bookmarkedColumnsKey, _bookmarkedColumnIds.toList());
  }
  ...
  Future<List<ColumnModel>> getRelatedColumns(String columnId,
      {int limit = 6}) async {
    _ensureInitialized();
  ...
  Future<ColumnStats> getColumnStatistics() async {
    ...
    final totalViews =
        _columnViewCounts.values.fold<int>(0, (sum, count) => sum + count);
    ...
    if (mostViewedColumnId != null &&
        _columnsCache.containsKey(mostViewedColumnId)) {
  ...
  Future<void> _syncWithServer() async {
    ...
    try {
      ...
      if (_favoriteColumnIds.isNotEmpty) {
        ...
        debugPrint(
            '$_moduleName: Syncing ${_favoriteColumnIds.length} favorites');
      }
      ...
      if (_readColumnIds.isNotEmpty) {
        // In a real app, this would sync with server
        debugPrint(
            '$_moduleName: Syncing ${_readColumnIds.length} read columns');
      }
  ...
  Future<void> removeMultipleColumnsFromFavorites(
      List<String> columnIds) async {
    _ensureInitialized();
  ...
  Future<void> shareColumn(ColumnModel column, {String? message}) async {
    ...
    final shareText =
        message ?? 'اقرأ مقال "${column.title}" بقلم ${column.columnistArName}';
